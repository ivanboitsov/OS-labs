...
## Решение
Для решения задачи были написаны два варианта программы на языке C++ для многопоточной четно-нечетной сортировки массива целых чисел по методу Бетчера.

### Первый вариант решения
1. Использует `CreateThread` и `WaitForMultipleObjects` для управления потоками в Windows.
2. Четные и нечетные фазы сортировки выполняются в отдельных потоках.
3. Синхронизация осуществляется с помощью мьютекса (`CreateMutex`), который защищает доступ к общему массиву.

### Второй вариант решения
1. Использует стандартную библиотеку потоков C++ (`std::thread`) и синхронизацию через `std::mutex` и `std::condition_variable`.
2. Ограничение числа активных потоков реализовано через счетчик `activeThreads` и условную переменную `cv`, что позволяет управлять конкурентностью.
3. Четная и нечетная фазы сортировки также выполняются в отдельных потоках, обеспечивая параллелизм и контроль числа работающих потоков.
...
управлении потоками в Windows и обеспечении синхронизации между ними.